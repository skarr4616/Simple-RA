...
public:
    ...
    void appendRowToPage();
    void reWritePage(vector<vector<int>> &mat);

    Page(string tableName, int run, int subfile, int pageIndex);
    Page(string tableName, int run, int subfile, int pageIndex, vector<vector<int>> rows, int rowCount);
};