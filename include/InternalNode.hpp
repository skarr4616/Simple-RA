...
public:
    ...
    /* Delete Functions */
    TreePtr getLeftSibling(const Key &key);
    TreePtr getRightSibling(const Key &key);

    bool canRedistribute(TreePtr a, TreePtr b);
    int updateTree(TreePtr parent_ptr = NULL_PTR);

    void redistribute(TreePtr a, string type) override;
    void merge(TreePtr a, string type) override;
